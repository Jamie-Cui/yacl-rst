...
use crate::{Result, EnvelopeError};
use yacl_pke::{RsaOaepDecryptor, RsaOaepEncryptor, Encryptor, Decryptor};
...
impl Sealer for RsaAes128GcmSealer {
    fn seal(&self, plaintext: &[u8]) -> Result<SealedMessage> {
        ...
        let (ciphertext, tag) = aead.encrypt(plaintext, &[], &nonce)
            .map_err(|e| EnvelopeError::DataEncryptionFailed(e.to_string()))?;

        // Encrypt the symmetric key with RSA-OAEP
        let encrypted_key = self.encryptor.encrypt(&symmetric_key)
...
impl RsaAes128GcmOpener {
    ...
    pub fn new(rng: &mut (impl rand::CryptoRng + rand::RngCore), bits: usize) -> Result<(Self, RsaAes128GcmSealer)> {
...
impl Opener for RsaAes128GcmOpener {
    fn open(&self, sealed: &SealedMessage) -> Result<Vec<u8>> {
        ...
        let symmetric_key = self.decryptor.decrypt(&sealed.encrypted_key)
        ...
        let key: [u8; 16] = symmetric_key.try_into()
        ...
        let nonce: [u8; 12] = sealed.nonce.clone().try_into()
