...
use aes::Aes128;
use cipher::{KeyIvInit, BlockEncryptMut, BlockDecryptMut, block_padding::NoPadding};
use cbc::Encryptor;
use cbc::Decryptor;
...
impl Aes128Cbc {
    ...
    pub fn encrypt(key: &Aes128Key, iv: &Aes128Iv, plaintext: &[u8]) -> Vec<u8> {
        ...
        let result = encryptor.encrypt_padded_mut::<NoPadding>(&mut buf, plaintext.len())
    ...
    pub fn decrypt(key: &Aes128Key, iv: &Aes128Iv, ciphertext: &[u8]) -> Vec<u8> {
        ...
        let result = decryptor.decrypt_padded_mut::<NoPadding>(&mut buf)
            .expect("invalid length");
        result.to_vec()
    }
}
