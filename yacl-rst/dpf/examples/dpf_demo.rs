...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("\n[KEYS] Key Generation:");
    println!(
        "  [OK] Key 0 (Party 0): rank={}, control_words={}",
        key_0.get_rank(),
        key_0.cws_vec.len()
    );
    println!(
        "  [OK] Key 1 (Party 1): rank={}, control_words={}",
        key_1.get_rank(),
        key_1.cws_vec.len()
    );
    ...
    let (key_0_small, key_1_small) =
        small_dpf.generate_keys_internal(&small_alpha, &small_beta, rng.gen(), rng.gen(), true)?;
    ...
    for i in 0..shares_0.len() {
        ...
        let marker = if i == small_alpha.get_val() as usize {
            "[*]"
        } else {
            "  "
        };
