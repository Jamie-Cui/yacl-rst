...
use ecc::{CurveName, EcGroup, Point, Scalar, P256};
...
impl OprfConfig {
    ...
    pub fn from_context_string(s: &str) -> Result<Self> {
        ...
        let dash_idx = rest
            .find('-')
            .ok_or_else(|| OprfError::InvalidConfig(format!("Invalid context string: {}", s)))?;
        let mode_str = &rest[..dash_idx];
        let cipher_suite_str = &rest[dash_idx + 1..];

        let mode =
            OprfMode::from_u8(mode_str.parse().map_err(|_| {
